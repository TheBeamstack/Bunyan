...
1. The parametric recipe is the source of truth; geometry and views are derived and disposable.
   ⚠⚠ **SWEPT 2026-07-27 (Entry 62, D74) AND IT WAS DIRTY, TOGETHER WITH RULE 3.** The sweep's form is *enumerate everything persisted and ask of each "is this a RESULT?"* — and `scene.brokenRefs` is the one field in `scene.json` that is one (persisted deliberately, so that closing a file cannot "fix" a model by forgetting its problem). It was re-derived only for the assemblies being rebuilt, and a **deleted** element is never a rebuild root ⇒ its entry survived every rebuild, **including `rebuildAll`, the primary load path**, because the thing it describes is not among the things there are to rebuild. ⇒ **A derived value that outlives its subject is no longer derived — it is stored, and it will eventually contradict the recipe.**
2. Sub-shape references are stable identities derived from provenance, never positional indices.
   ⚠ **SWEPT 2026-07-27 (Entry 62 cont.) AND CLEAN, by counting:** every positional read of a `refs` array in production code was enumerated, and there is **one** — `protocol/src/mesh.ts` mapping a triangle to a ref *index* within a single tessellation message. That is a wire format inside one payload, not an identity that outlives it. Everything that selects a face selects it **by role name** (`facesWithRoles`, the resolver, the Types), because the array is canonically ordered by the kernel and **its order is not a contract — the role is** (D26).
3. Broken references fail loudly (marked, manually retargeted), never silently reattach.
   ⚠⚠ **SWEPT 2026-07-27 (Entry 62, D74) AND IT WAS DIRTY.** The load-bearing words are *"awaiting manual **retargeting**"*: a broken ref left behind by a **deleted** element awaited nothing — `core.retargetReference` cannot act on an element that does not exist — and it was **saved into the `.bnn`**, so a document was left permanently and unfixably dirty: a UI showing a fault with no way to clear it, and any consumer asking *"is this model clean?"* reading dirty for the life of the file. ⇒ **A refusal nobody can act on has stopped being a refusal and become a lie about the model's state.** Failing loudly is necessary and not sufficient; the refusal must stay *actionable*, and it must die with the thing it describes.
   ⚠ **A SECOND CLASS SINCE D86, AND IT IS NOT RETARGETABLE:** an element naming a `designOptionId` the document does not define is reported by `brokenRefs()`, and `core.createElement` is the only verb that writes the field, so no verb clears it until the option catalogue can be authored (D85, `docs/BACKLOG.md` T-011). It satisfies *"actionable"* by being **derived at the query and never stored**: it enters no `.bnn`, and it clears the moment the option resolves.
   ⚠ **A THIRD CLASS SINCE D83:** an element naming a `hostId` or `parentElementId` that is not an element in this document is reported the same way, derived and never stored. `core.retargetReference` clears the host edge and the parent edge can be pointed at a live element but not cleared, which is affordable because the delete road is closed by the cascade (D83's (a) half) and what reaches this state is a document written elsewhere (D43).
4. Failed operations reject and preserve the last-good state; no partial or auto-invented geometry.
   ⚠⚠ **SWEPT 2026-07-27 (Entry 62 cont., D75) AND IT WAS DIRTY — AT THE GRANULARITY THE RULE IS ACTUALLY ABOUT.** The single-edit case was sound (D42's staged, all-or-nothing rebuild). The AGGREGATE was not: `projectQuantities` measured every element unguarded, so **one element that could not be measured threw out of the loop and the whole building's take-off returned nothing** — not even the `unmeasured` list that exists for exactly that case, and against an explicit ruling that an unmeasurable element is *reported*, never dropped. ⇒ **"Preserve the last-good state" has to be read against the SCOPE of the operation: for one element the last-good state is nothing, but for an aggregate it is *every other element*.** A query that answers for 9,999 elements and names the one it could not is correct; one that answers for none is the partial result this rule forbids, inverted. *(And the reachable cause was a Type declaring a billable face it did not have — now refused at build time, because silently skipping it would under-report an area still wearing `basis: exact`, which is rule 15's failure by a new road.)*
...
7. Units are millimetres internally; interchange declares its own units at the boundary.
   ⚠ **SWEPT 2026-07-28 (Entry 64) AND CLEAN, by counting the BOUNDARIES rather than reading the code.** Every crossing declares: the frozen kernel protocol says *"Angles are DEGREES"* and `kernel.cpp` converts to radians **and range-validates `(0,360]`** at the crossing; the Clean Delta converts mm³→m³ / mm²→m² / mm→m under `units:'metric'`; every reserved surface that freezes at P5 (documentation `world mm`, families *"a number (mm, per rule 7)"*, `georeference`, constraint values *"mm or degrees"*) declares in its own doc; and of 49 numeric `ParamSchema` leaves, 40 declared `unit` and the 9 that did not were 5 dimensionless counts/indices plus 4 coordinate arrays declaring mm in prose. ⚠⚠ **BUT IT WAS CLEAN ONLY BECAUSE EVERYONE REMEMBERED — the condition under which all nine dirty rules were found dirty.** Unlike rules 9/10, nothing made an omission loud. `tests/units-rule7.test.ts` now enumerates every numeric leaf and fails on any that ships without a unit, against a NAMED dimensionless allow-list; the 4 coordinate arrays were made machine-readable (`unit:'mm'` on their items) so an agent need not parse a description. *A clean rule with no enforcement is a dirty rule that has not happened yet.*
8. Nothing in the current version may foreclose a declared north-star (§9).
   ⚠⚠ **SWEPT 2026-07-28 (Entry 64) AND IT FOUND ONE — AND THE ROAD IS THAT A RESERVATION AGES.** The sweep's form is *the reservations were made on a date; what has been BUILT since, and does any of it foreclose?* (D58–D66 landed 07-21→07-24; D67–D75 and Entry 63 all landed after). **The find: `BuiltPart.exposedRefs` (D72, 07-27) — the member that answers *"what area do I bill?"* — is TYPE-ONLY knowledge by design, and the D61 FAMILY GRAMMAR (07-22, freezing at P5) has no slot for it.** `FamilyPart` carries `name`/`materialId`/`discipline`/`base`/`modifiers` and nothing that names a billable face ⇒ **every data-authored family can only ever report the absent-defaulted whole-solid area** — the number D72 itself measured at 1.07×–3.03× wrong and called *"a wrong number wearing the `exact` badge"*. The families north-star (§9, Revit's Family-Editor moat) would ship permanently unable to bill correctly, and after step 6 the slot is a three-product amendment. ⚠ **NOT fixed here: the grammar shape (how DATA names a face — role names on a primitive) is a design ruling, not a mechanical fix.** SURFACED for the owner. *(The counter-example the same sweep produced: this session's own rule-14 guard was written scalar-only and the D60 co-authoring test refused it — a reservation defending itself. That is what rule 8 looks like when it works.)*
9. **There is exactly one command layer.** Every actor — human or agent — acts on the Document only through Commands (§3.13); no actor has a private path to the kernel. Every Command **returns** the state delta it produced, so every action is verifiable. **⚠ ISSUING A REVISION IS A COMMAND** *(D41)* — it is an actor acting on the Document, and there is no second path. **And every Command can be run as a `dryRun`** *(D42)*: full fidelity, same code path, result discarded — so *"what would this do?"* is answered by **doing it**, not by a second, hand-written description of it (rule 10).
    ⚠ **SWEPT 2026-07-27 (Entry 62) AND CLEAN — the first rule swept clean without needing a fix, along with rule 10.** Both mechanical halves hold **by construction**: *"every Command returns the state delta it produced"* because `applyChanges(scene, edit.changes)` is the **only** way the scene moves, and *"every Command can be run as a `dryRun`"* because `dryRun` is one branch in the **executor**, returning before the scene is swapped, before the journal, before the undo push. **A command cannot opt out of either, because a command never knew about them.** ⚠ **One finding recorded, not acted on: `undo()`/`redo()` are things an actor does to the Document and are not Commands** (no `argsSchema`, absent from `listCommands()`, though exposed on the agent surface) — D41's exact argument shape. It is **additive** (a new verb is a registration, rule 5) and therefore forecloses nothing at the freeze; an owner call at leisure.
10. **The model describes itself.** Types and Commands are discoverable at runtime from the registries that govern them (§3.13); there is no separately-maintained description of what the app can do.
    ⚠ **SWEPT 2026-07-27 (Entry 62) AND CLEAN, by counting rather than reading:** every command's `argsSchema` was compared against the keys its `execute` actually reads (29 commands, zero mismatches), and every shipped Type's `parameterSchema` against what its builders read. ⚠ **The reason it held is worth more than the result: `validateParams` REFUSES an undeclared argument**, so a schema that drifted from its implementation is a hard refusal, not a silence. *Compare rule 5, which quantified over four registries and had no such enforcement — two of the four were decorative for months.* ⇒ **a rule holds when violating it is loud; the rules swept dirty here were all enforced by memory alone.**
11. **An element owns an ordered list of PARTS; a part belongs to exactly one element.** *(D30.)*
    ⚠ **SWEPT 2026-07-27 (Entry 62 cont.) AND CLEAN, by counting the `boolean` op call sites that actually build — there are exactly TWO:** `build.ts` cutting a hosted void through its host's parts (the sanctioned intra-assembly case) and `core.opening` building its frame as `outer − inner` (intra-element). **No boolean anywhere in the product takes two ELEMENTS' solids as operands.** *The anti-fuse rule is the one this codebase has been most careful about, and it is the only rule that was already load-bearing enough to be checked by counting two call sites.* Never fuse two **elements** — that re-owns the first one's faces and retroactively breaks every reference hosted on it. But an element is **not** one solid: it is its parts, each with a material, and a boolean **among an element's own parts** (an opening cutting every layer of its host) is an ordinary intra-element operation. **A single-solid element is just an element with one part.**
...
14. **A model is ISSUED, not merely saved.** *(D34.)* A **Model Revision** is a deliberate snapshot; deltas are computed against it. **`change_type` is READ from the edit log, never inferred by diffing geometry** — a tool that guesses what changed will eventually guess wrong, and downstream that is a wrong schedule.
    **⚠⚠ AND THE LOG THAT SENTENCE MEANS IS THE *JOURNAL*, NOT THE UNDO STACK** *(D40, §3.12a)*. The journal is **append-only, unbounded, never popped by an undo**, and a Revision records **`issued_at_seq`**, the position it was issued at. **A bounded, mutable, unanchored stack cannot answer "what changed since revision N", and a rule that rests on one is a slogan.** *(It rested on one for an entire phase, and nobody noticed, because the undo stack was in the same file and had the same shape.)*
    ⚠⚠ **SWEPT 2026-07-28 (Entry 64, D76) AND IT WAS DIRTY — NOT IN THE MECHANISM, WHICH IS SOUND AND TESTED, BUT IN THE PRECONDITION UNDERNEATH IT: NOTHING EVER CHECKED THAT THE LOG A CONSUMER HOLDS REACHES THE BASELINE IT IS READ AGAINST.** `issued_at_seq` names a position in the journal; `since()` on a journal that cannot see that position returns `[]` — **indistinguishable from the true and ordinary answer *"nothing has happened since I issued it."*** Measured on a wall that grew 6 m → 8 m after the baseline: a `.bnn` saved with its revision but no `history.json` reopened and published a valid `contract_version: 1.2` package with **zero elements and an all-zero summary**, and a `.bnn` saved with `doc.history()` (the 200-deep undo stack) in place of the journal **dropped an entire wall built after the baseline out of the delta** once the stack had shifted. Planitor reads absence-from-`elements` as *unchanged* ⇒ it keeps billing the model it already has. ⇒ **THE RULE'S FAILURE MODE IS NOT A WRONG GUESS — IT IS A MISSING LOG REPORTED AS SILENCE**, and silence is the one answer a change feed must never be able to fake. ⚠ **The detector is structural, and D41 is why: an issued revision is journalled but NEVER pushed onto the undo stack**, so a log that is really the undo stack *cannot* contain the anchor — the mistake the docs had warned about in prose since D40 is the one thing the check is guaranteed to see. Fixed at all three sites (`changesSince`, the Clean Delta exporter, and `saveBnn`, which now refuses to WRITE a file whose journal contradicts its own manifest); an absent journal stays legal (a scene-only `.bnn` is a real document) and is refused only when asked for a delta.
...
17. **A DRAWING IS A PROJECTION OF THE B-REP, NEVER AN AUTHORED ARTIFACT.** *(D58.)* Every plan, section, elevation, schedule, tag, and dimension is a **live view computed from the model** — a `sectionCut`, a query, a `SubShapeRef`/vertex anchor — regenerated when the model changes, never the source of truth. A hand-drafted line not backed by geometry is the **same failure class as a mesh treated as authoritative** (§10): it drifts and lies. ⚠⚠ **This is the capability Revit exists to sell** — a coordinated construction-document set is the architect's actual deliverable, not a 3D model. **v1.0.0 ships the MINIMAL projection (one plan, one section, one schedule); the full documentation apparatus is the largest single item on the road to parity** (`v1.0.0_imp_plan.md` "Road to parity"). ⇒ **its anchoring contracts — a View descriptor, a Dimension/Tag on a `SubShapeRef`+vertex, a Schedule bound to type/param/quantity, a Sheet/Viewport — are RESERVED at the P5 freeze**, because foreclosing them is the most expensive foreclosure available.
    ⚠ **SWEPT 2026-07-28 (Entry 64) AND CLEAN on everything that exists** — and it is clean *by shape*, which is the strong form: a `Dimension` has **no value field at all** (its number re-derives from its anchors), a `Tag` stores a `subject` KEY and not its text, a `ScheduleDefinition` stores a filter + column keys, and a `ViewDescriptor` stores only how to PROJECT (cut plane, direction, scale, clip) — the one place a stale drawing could hide is a stored result, and there is nowhere to store one. **⚠⚠ ONE FINDING SURFACED, NOT FIXED (owner's call, and cheap only until step 6): `Dimension.anchors` accepts the `point` member of `AnnotationAnchor`** — a free 2D point on paper, correctly reserved as *"the escape hatch for annotation that references no geometry… it does NOT survive a model edit, by design."* That is right for a text note and wrong for a **dimension**, which is the one annotation a builder reads AS a measurement: two paper anchors give a number no model edit will ever update — this rule's own *"drifts and lies"*, on the artifact it names. Narrowing `Dimension.anchors` to the model-anchored members is one line while the shape is release-candidate; after the freeze it is a three-product amendment, and the P6 body author will have implemented it by then.
...
18. **AN ELEMENT MAY OWN CHILD *ELEMENTS*, NOT ONLY PARTS.** *(D59.)* A Part is a solid inside one element (rule 11); but a **curtain wall** is panels + mullions on a grid, a **stair** is treads + stringers + railings, a **group/assembly** is elements-of-elements — each child a first-class element with **its own PEI**, hosted by its parent. Hosting is therefore **not one level deep** (opening→host is only the first case). The frozen model must carry **composition** as a real relationship, and — per the owner ruling (D59) — the **complete composition/hosting model is DESIGNED before the freeze**, not merely reserved, so a curtain wall / stair cannot become a frozen-contract amendment. ✅ **BUILT (2026-07-22, Model A — `P5_step5B_composition_nesting_design.md`):** a parent GENERATES its children from its recipe (never stored `scene.elements` rows — recipe-is-truth); each child's PEI is DERIVED (`${parentId}:${slot}`, the `nodeId`/`lateral.k` discipline one level up), so a curtain wall is ONE authored row whose panels/mullions regenerate each rebuild and a tag on a vanished slot is a broken-ref (the D1 story, one level up). The carrier is `BimObjectType.buildChildren?` + `ElementGeometry.children?` (a tree); **`parentElementId` is re-pinned to the MANUAL group/nest case** (a flat `groups` collection is additive v1.0.x). ⚠ **The anti-fuse rule (rule 11) still binds and HELD**: composing elements never fuses their solids; a nested element's references survive its siblings changing (the real `core.curtainwall`, depth-2, proved it vs OCCT).
    ⚠⚠ **SWEPT 2026-07-28 (Entry 64, D77) AND IT WAS DIRTY AGAINST THE INVALIDATOR — the half of "a child is a first-class element" that nobody had followed through.** `BuiltChild.styleId` is a real member (*"a panel style shared across every panel"* — D31 one level down) and `contextFor` resolves it into a child's `BuildContext` exactly as for an authored row, **so the build READS a child's style.** The style→instance edge answered *"every element wearing this style"* out of `Object.values(scene.elements)` — and **a Model-A child is not a scene row.** Measured on real OCCT: `core.updateStyle` on a style worn only by children reported **`rebuilt: []`** and left their solids **3× stale** (20,000,000 mm³ where 60,000,000 is correct), published through `projectQuantities` wearing `basis: 'exact'` — **until some unrelated `rebuildAll` silently cured it**, so the live session and the saved file disagreed and only the file was right. The section edge inherited it verbatim (a section reaches geometry only through a style). ⇒ **`dependency.ts`'s own rule — *an edge the build READS must be an edge the invalidator KNOWS* — did not survive composition**, because which children exist is produced by a Type's code and no pure function of `scene.json` can know it. Fixed by giving the graph the built tree (`childStyleUsers`) and re-staging the **authored root**, which regenerates the child. *A rule that adds a new KIND of element must be swept over everything that quantifies over "every element" — the invalidator, not only the consumers.*

19. **A TOOL COLLECTS INPUT; ONLY A COMMAND CHANGES THE MODEL.** _(D47, P4.5 — owner-ruled 2026-07-30, `P4.5_interaction_model_design.md` Q1.)_ A **tool** is a UI state machine that gathers arguments — a point, a face, a length — and **commits exactly one `Command`**. It is never a second command layer, it never holds a `KernelClient`, and **an in-progress tool interaction is not model state**: nothing half-drawn is ever in the document, so a refresh, an undo, a save, or an agent query can never observe it. ⇒ **A preview is not a `dryRun` and a `dryRun` is not a commit** — the rubber-band line a user drags is overlay geometry the renderer owns, and the only thing that ever reaches the document is the single `Command` dispatched at the end. ⚠ **This keeps rule 9 intact rather than qualifying it:** there is still exactly one command layer, and a tool is strictly *upstream* of it — which is why the machine-check is the P4 equivalence test run against a **tool-authored** edit (the same edit through `window.bunyan` must yield the same `UndoableEdit`). Without this rule the tool layer is the natural place for a private authoring path to appear, because it is the only layer that holds half-finished intent.
    ⚠ **NUMBERED 19, NOT 17, AND THE REASON IS A CORRECTION WORTH KEEPING.** The design doc proposed "rule 17", believing the only collision was a `schedule.ts` code-comment convention of that name. It was wrong: **17 (a drawing is a projection, D58) and 18 (an element may own child elements, D59) are both real, swept domain rules**. The owner ruled *adopt*, and the number follows the facts. *The check that caught it is this project's own: read the list rather than the claim about the list.*

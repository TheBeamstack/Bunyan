...
| **Standalone** | Miqdar alone, ETABS-style. Import from IFC (structural view) when available; export to `.bnn`/IFC at the end if wanted. |

### 3.2 Interop transport

- **v1.0.0: file-based.** Miqdar reads and writes `.bnn` (Bunyan's documented package — `scene.json` is semantic truth, which is exactly why this works) and keeps its own `.miqproj` (§6.5). No live link.
- **⚠ NEW — AND MIQDAR IS NOT THE ONLY CONSUMER ANY MORE (Bunyan D34, 2026-07-13). THE ECOSYSTEM HAS FOUR PRODUCTS AND *ONE* CHANGE FEED.** Beside the full model (`.bnn`) there is now a **change feed**: the **Clean Delta Package** — *what changed since revision N* — an **existing contract** shared by **Planitor** and **BIMsync** (`Planitor/v2.2_spec.md` §4), which **Bunyan now emits natively** (`source: "bunyan"`). ⇒ **Miqdar should consume it too**, and for the obvious reason: *the architect issues a revision; which of my analysed elements actually changed?* **Re-analysing a whole tower because one beam moved is the workflow engineers hate**, and the delta is exactly the answer. **Do not build a second differ** — read the feed.
- **⚠ AND THE FEED IS *TRUSTWORTHY* FROM BUNYAN IN A WAY IT CANNOT BE FROM ANY IFC PRODUCER.** Its `change_type` is **read off Bunyan's edit log**, not inferred by diffing geometry; `reidentified` **cannot occur**; and `quantity.basis` is **`exact`** — measured on the B-Rep, **per part, per material**. *(For FOREIGN models, **BIMsync** manufactures the same package the hard way — minting a PEI, fingerprinting, and escalating ambiguity to a human queue. Same contract, weaker guarantees. Miqdar reads both and need not care which.)*
...
- **Sub-shape binding:** `SubShapeRef` — a support at *the base face of that column*; a line load on *the top edge of that wall*. **Because of Bunyan D1, these bindings survive the host being resized, re-parameterized, and rebuilt from scratch.** This is the single technical property that makes the round-trip trustworthy, and it is the property the Revit↔RSA link famously lacks.
  - **⚠ AND THERE IS NOW EXACTLY ONE BOUNDED EXCEPTION, WHICH MIQDAR MUST KNOW ABOUT: Bunyan's D28 positional key** *(owner ruling 2026-07-13)*. Where two sub-shapes are **genuinely topologically symmetric** — the two rims of a duct drilled clean through a **round column** — no structural rule can separate them, and Bunyan orders them by a **mm-rounded centroid**. **A recipe edit that moves the geometry across the tie can therefore swap the two.** It is the *only* place geometry touches Bunyan's identity path. **⚠ Bounded by a structural fact, and this is the part that matters to Miqdar:** Bunyan's `transform` mints **no** identities, so the key is computed in the element's **own build frame** ⇒ **moving or rotating a column anywhere in the world cannot re-rank anything.** ⇒ **A Miqdar binding onto a symmetric-tie sub-shape carries D28's caveat; every other binding carries the full D1 guarantee.** *(Structurally this is a rare surface to bind to — a support sits on a base face, not on a duct rim — but the guarantee above was stated without qualification, and after D28 that was no longer strictly true.)*
- **⚠ Bind to a STYLE, not to N instances, wherever the engineer thinks in groups** *(Bunyan D31)*. Bunyan's `ElementStyle` is a named, shared parameter set (`IPE300`) that many elements reference; **it is the direct counterpart of Miqdar's `DesignGroup`.** A section change that Miqdar decides *once* for a group is written *once* to Bunyan. **This is new — it did not exist when §3.4 was first written**, and without it the flagship round-trip would have had to set a property on each instance individually.
...
| 2 | **Keep `.bnn`/`scene.json` documented and semantic** — already promised (spec §6). | zero |
...
| 6 | **✅ DISCHARGED — `core.beam` IS IN BUNYAN v1.0.0 (Bunyan D32, owner ruling 2026-07-13).** This row used to *request* a Beam for a later Bunyan 1.0.x, and Bunyan's plan explicitly answered *"do not add Beam on Miqdar's account."* **Both were wrong.** Bunyan now ships **`LinearMember → Beam · Column`** — one concept (a Section swept along an axis, with a cardinal point), two specialisations, exactly as IFC and as an engineer see it. ⚠ **AND IT WAS NOT ADDED ON MIQDAR'S ACCOUNT** — it is in because a BIM authoring tool without a beam is not a BIM authoring tool, and Bunyan already shipped a Column, which is the identical concept rotated. **The no-coupling rule stands unamended.** ⇒ **The lossy-beam fallback is VOID; the structure-first round-trip is full-fidelity at launch.** | **zero — already in Bunyan v1.0.0** |
| 7 | **✅ NEW, AND IT IS A GIFT, NOT AN OBLIGATION: Bunyan now has `ElementStyle`, `Material` and `Section` as first-class shared entities (Bunyan D31/D33).** An `ElementStyle` is a **named, shared parameter set** (`IPE300`, `EXT-200-Concrete`) that many instances reference, and editing it rebuilds them all. **⚠ THIS IS THE COUNTERPART OF MIQDAR'S `DesignGroup`, AND IT DID NOT EXIST WHEN THIS SPEC WAS WRITTEN.** Without it, Miqdar's *"assign one section to a group of columns"* would have had to write a property onto each instance one at a time — which is exactly the workflow engineers do not use, and it would have made the flagship round-trip feel broken even though every byte was correct. **Miqdar binds a DesignGroup to a Bunyan Style; a Material carries `f_ck`/`E`/density as real properties rather than a string.** | **zero — Bunyan built it** |

> ✅ **BOTH HALVES OF THIS NOTE ARE NOW OBSOLETE — AND IT IS WORTH RECORDING WHY, BECAUSE THE CHAIN IT WARNED ABOUT IS *GONE*, NOT MERELY SATISFIED.** It read: *"the beam fallback is gated on Bunyan's extrude op landing"* (true when written — Bunyan Entry 9 had **no prism, no sweep, and the GenericSolid escape hatch had no op behind it**). **(1)** Bunyan built **`extrude`** (Entry 12) and **`revolve`** (Entry 14), so **GenericSolid works** and the fallback is real. **(2)** But the fallback is **no longer needed at all**: Bunyan ships **`core.beam`** in v1.0.0 (**D32**, row 6 above). ⇒ **There is no lossy channel, and no dependency chain.** *(Kept, not deleted, because it is the record of a real coupling that was correctly identified and has since been dissolved.)*

| 8 | **✅ NEW AND UNASKED-FOR: Bunyan now tells Miqdar WHICH ELEMENTS ARE STRUCTURAL (Bunyan D36).** ⚠ **This spec said, from its first draft, that Miqdar imports *"the structural elements"* — and NOTHING IN BUNYAN COULD ANSWER WHICH THOSE WERE.** A Wall may be a **shear wall or a partition**; that is a **property, not a type**. Miqdar would have had to **guess** — and a guess yields a **wrong structural model** rather than a refused one, which is the single outcome §10 exists to prevent. Every Bunyan element now carries **`loadBearing`** + **`discipline`** + its IFC class. **⇒ Miqdar's import filter is now a READ, not an inference. It must never guess again.** | **zero — Bunyan built it** |
| 9 | **✅ NEW: the spatial container tree (Bunyan D35)** — `Site → Building → Level → Space`. Miqdar can locate a frame ("Tower A, Level 3") rather than a soup of coordinates, and it is the same tree Planitor schedules against. | **zero — Bunyan built it** |
...
├── link.json            # Bunyan bindings: source .bnn identity, instanceId/SubShapeRef map, sync state
...
Results are rebuildable from `model.json`; a corrupt cache loses nothing (M2). The solver build id gates the cache exactly as Bunyan's kernel build id gates *its* geometry cache. ⚠ **Note (Bunyan D29, 2026-07-13): Bunyan's `geometry-cache.brep` was never built, and whether it ships at all is deliberately undecided** until its rebuild cost can be measured. **The principle is what Miqdar adopts, not the file.** ⚠ **And Miqdar is a stakeholder in that decision:** Miqdar **writes** `.bnn` (the structure-first path) and has a *solver*, not an OCCT kernel — **it cannot produce a BREP cache even in principle.** ⇒ **Bunyan's loader must work with NO cache present.** That is not merely Bunyan's internal invariant; it is a hard requirement imposed by a second product, and it is an argument for dropping the cache.
...
**The end-to-end Miqdar v1.0.0 acceptance task (human):** model a 5-storey RC frame-and-shear-wall building on a Moroccan site (RPS 2000 v2011 zone, site class, ductility level authored); generate combinations; run modal + RSA; design beams, columns and walls to the Moroccan pack; run `optimize` on the column DesignGroups **under an authored `maxUtilization` (§7.1)**; review the diff; sign off; generate the *note de calcul* (FR); **export the structural model to `.bnn` and open it in Bunyan**. Then the same building under the Algerian pack, and under the French pack. *(Which editions — per the normative register, §5.2.)*

> ✅ **THIS LIMITATION IS VOID — AND IT SHOULD NEVER HAVE BEEN ACCEPTED.** It read: *"the export step lands beams as **GenericSolid**, not as `core.beam` — Bunyan has no Beam type at its v1.0.0. The round-trip is therefore geometrically correct and **semantically lossy** at Miqdar v1.0.0, and that is **ACCEPTED**, not a failure of S7."* **Bunyan now ships `core.beam` in v1.0.0** (`LinearMember → Beam · Column`, Bunyan **D32**, 2026-07-13). ⇒ **The acceptance task's export step is FULL-FIDELITY: beams land as beams, columns as columns, each carrying a catalogued `Section` and a shared `ElementStyle`.** ⚠ **Recorded rather than deleted, as the clearest example of the failure mode both specs exist to prevent:** *the ecosystem's flagship workflow was quietly written down as lossy, and the word "ACCEPTED" made it look like a decision instead of a gap.* **A gap in the ecosystem is not a Miqdar limitation to absorb — it is a Bunyan feature to build.**
   ...
   Bunyan v1.0.0 ships
        └──►  Miqdar development STARTS          (prerequisite gate, owner direction: "afterwards")
                   └──►  Miqdar S7 (Bunyan round-trip)
                              ├── needs Bunyan's EXTRUDE op        ── ✅ BUILT (Bunyan Entry 12),
                              │                                        + REVOLVE (Entry 14)
                              ├── needs Bunyan `core.beam`         ── ✅ IN BUNYAN v1.0.0 (D32) —
                              │                                        NOT a later 1.0.x any more
                              └── wants a DesignGroup counterpart  ── ✅ `ElementStyle` (D31),
                                                                       + Material/Section registries (D33)
```

**✅ THE CHAIN IS DISCHARGED — ALL THREE LINKS, AND TWO OF THEM BY RULINGS MADE *BECAUSE* THIS DIAGRAM EXISTED.** The original read: *"S7 needs Bunyan's extrude op — **NOT YET BUILT**"* and *"S7 **wants a Bunyan 1.0.x** `core.beam` — a release **later** than the one that unblocked Miqdar."* Both are now false, in Miqdar's favour: **`extrude`/`revolve` are built**, and **`core.beam` is in Bunyan v1.0.0**, not in some later release Miqdar would have had to wait for. ⇒ **Miqdar's S7 depends on nothing that does not already exist in Bunyan v1.0.0**, and the structure-first round-trip is **full-fidelity at launch**.

⚠ **Nothing here couples the release schedules** (§3.4 forbids it, and the rule is unamended): **Beam went into Bunyan because a BIM authoring tool without a beam is not a BIM authoring tool** — Bunyan would have needed it had Miqdar never existed. **But the diagram is what made the gap visible**, and that is precisely the job it was written to do. *Keep drawing these.*
...
| **M16** | **Interop transport: file-based v1.0.0 (`.bnn`/`.miqproj`); live round-trip in v1.x via `window.bunyan`** — Miqdar drives Bunyan as an agent through the one command layer; the inter-app API *is* the agent API (D19's payoff). |

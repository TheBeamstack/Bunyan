...
│  Document model (scene graph + dependency DAG + the JOURNAL)       Persistence manager   Service worker         │
└───────────────┬─────────────────────────────────────────────────────────────────────────────────┬─────────────┘
...
### 5.5 Rebuild & load
- **⚠ CORRECTED 2026-07-13 (review; owner-ruled). THERE IS NO PERSISTED "IDENTITY TOKEN MAP", AND THERE MUST NOT BE.** This line used to claim one. `scene.json` persists only **the `SubShapeRef` tokens objects actually AUTHOR** — an Opening's `hostRef`, a fillet's target edge. Every other identity is **re-derived** by replaying the recipe on load, because the derivation is deterministic (spec §4.5). **Storing the full set would be storing a RESULT**, which the core invariant forbids and which buys nothing the re-derivation does not already give. *(Claimed in five places across two docs with zero code behind it — the shape of the BREP-cache bug, except here the **code was right and the prose was wrong**. Deleted, not built.)*
...
### 6.5 Save / load
- **Save:** serialize `scene.json` (**the recipe — that is all it is**), `manifest.json` (incl. OCCT build id, per-type versions, **and the Model Revision + its `issued_at_seq`** — D34/D40), **`history.json` (⚠ THE JOURNAL — append-only, `seq`-ordered, NOT optional and NOT the undo stack; D40)**, `thumbnail.png` into the `.bnn` zip — **plus `geometry-cache.brep` (D29: ✅ RULED **SHIP**, owner 2026-07-14 — ⚠ protocol ops RESERVED, bodies NOT yet written; spec §6).** ⚠ The cache carries, per part, the BREP bytes **+ its identity tokens in CANONICAL order + a fingerprint** — it does **not** carry a token map keyed by geometric index (D1). See spec §6's D29 box.
- **Load:** read manifest; **rebuild from `scene.json`** — which is the ONLY path that exists today, is the **primary** path forever (the recipe is truth), and is the only path **Miqdar** can ever take (it has a solver, not an OCCT kernel — it *cannot* produce a cache even in principle). **With the cache (D29):** `importBrep` re-attaches each part's tokens **by canonical order** and **verifies a fingerprint** taken at write time; **any** disagreement — a different kernel build id, re-ordered topology, a corrupt or hostile file — yields **`CACHE_STALE`**, the cache is discarded, and this same rebuild runs. ⇒ **the cache can never mis-name a face; it can only fail to save time.** Then run per-type migrations and deterministically re-derive tokens.
...
`.bnn` (zip) = `manifest.json` + `scene.json` + **`history.json` (THE JOURNAL — the ecosystem's change feed, D40; not optional, never trimmed, an undo appends a REVERSAL rather than erasing)** + optional `thumbnail.png` **+ `geometry-cache.brep` (D29 ✅ RULED **SHIP** — owner, 2026-07-14, with the measured 7.3 s cold load in hand; ⚠ **RESERVED in the protocol, bodies not yet written**)** (spec §6). **[v1.0.0] no `export.ifc`** (IFC export deferred; `core_logic.md`/spec D3). The recipe (`scene.json`) is truth; the BREP is a **rebuildable cache**; a stale/corrupt cache is **never treated as truth** — it is refused (`CACHE_STALE`) and the recipe rebuilds.
...
- **Import hardening.** `.bnn` zips: decompression size/ratio caps + entry-count limits (zip-bomb defense). IFC: schema validation + parsing inside the worker under a **timeout** so a hostile file cannot hang or OOM the tab.
- **Untrusted BREP cache.** ⚠⚠ **D29 SHIPS (owner, 2026-07-14), so this attack surface is now OWNED DELIBERATELY, and the hostile-BREP hardening test is a REQUIRED deliverable** (spec §9) — it was previously conditional, and dropping the cache would have deleted it outright. `geometry-cache.brep` is **the one part of a `.bnn` fed as BINARY to OCCT's deserializer**, and **a `.bnn` is a file a user can be SENT.** `importBrep` therefore treats its bytes as hostile: bounded, validated, and refused with a **typed failure** (`CACHE_STALE`) rather than a crash — after which the recipe rebuilds. ⚠ And the identity half of the same rule: a cached solid's tokens are re-attached **by canonical order and verified against a fingerprint**, so a tampered cache **cannot re-point a window at a different wall** — it can only be refused.
- **Untrusted recipe.** `scene.json` (and `history.json` — the **journal**, D40) is validated on load; malformed graphs fail safe rather than driving the naming engine into bad states. ⚠ **`typeof null === 'object'`** — a guard that tests `typeof x !== 'object'` does **not** reject a null collection, and a hostile `.bnn` sailed straight through the check written to catch it (found by review, 2026-07-13). **Validate shapes, not `typeof`.** ⚠ **An element whose Type is unknown, or whose `typeVersion` is from the FUTURE, is `failed` + PRESERVED VERBATIM — never built, never dropped** (D43): a round-trip must be lossless for a file we only partly understand, or saving a Miqdar model silently deletes its columns.

...
packages/
  ...
  document/     ★ @bunyan/document      THE PARAMETRIC TRUTH LAYER (Entry 18; corrected 21; D50 step-0 33–38)
                    ...
                    schedule.ts  ★ D58 row Ⓐ — THE SCHEDULES BODY (Entry 65). `ScheduleDefinition` -> rows:
                                     `evaluateSchedule` (the D19 door) = modelElements() -> selectRows -> a cell
                                     projector. ⚠ NO enumeration loop of its own — that IS its correctness.
                                     `columnKeyOf` is the one place the column-key grammar lives (owner Q1).
...
the real body; revert-verified five ways)** · D50 step
...
- **Amer:** unchanged (P4.5, FSA adapter, WebGPU, service worker/PWA, Cloudflare deploy, the optional `codecFor` wiring), plus the Entry-64 `saveBnn`/`BNN_CODEC.write` notes which still stand.

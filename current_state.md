...
| **`Command`** | **✅ WRITTEN (Entry 18)** — carries an **`argsSchema`** and `execute` **returns** its `UndoableEdit`. It is **the agent API** (§4f). ⚠ **The CRUD verbs carry the D51 refuse-or-retarget args** (`acknowledge`/`retargetMap`, Entry 35, row ⓓ) and **`createElement` now carries the six reserved-metadata args + `core.setElementMetadata` authors them post-create** (Entry 38, row ⓣ) — *both landed pre-freeze precisely because `argsSchema` freezes at P5.* | **P5** (**with** its `argsSchema`) |
| **`ElementStyle` / `Part` / `Material` / `Section` / spatial tree** | **✅ WRITTEN (Entry 18)** — `scene.json` is `packages/document/src/scene.ts`. ⚠ **`SpatialContainer`/`Grid` gained optional `properties?`/`classifications?` IFC bags** (Entry 38, row ⓤ) — additive, mirroring `Element`. | **P5** |
...
| **D52** | **✅ RULED (2026-07-16) — A WALL IS A BASELINE `{start, end}`; ITS LENGTH AND HEIGHT ARE DERIVED, NOT TYPED** (params: `start`, `end`, `baseLevel`, `topLevel`). Owner ruling ① of the Freeze Gate. *"Drag the wall's end"* is `setParams{end}`; a wall-to-wall **join** is two baselines meeting (never a fuse — D50 §0c); a **grid-host** snaps `start`/`end` to a named axis. This is the Revit/ArchiCAD model and the only parameterisation on which joins and grid-hosting are expressible. **Freezes the Wall's `parameterSchema` and the `move`/`setParams` command shapes** (P4.5 s4 = P5 s0b, answered once). |
| **D53** | **✅ RULED (2026-07-16) — CONSTRAINTS LIVE IN A FIRST-CLASS `scene.json` `constraints` COLLECTION, ORTHOGONAL TO `params`.** Owner ruling ② of the Freeze Gate. `ParamSchema` gains **no** driven/driving-dimension concept — params stay plain values; the typed dependency/constraint graph (P5 s0a) is the single structure the rebuild invalidator **and** Miqdar/Planitor read. Its edges are `container→element` (elevation), `grid→element` (axis), `host→hosted` (boolean), `style→instance`, plus the D50 constraint edges. |
| **D54** | **✅ RULED (2026-07-16) — RESERVE THREE OPTIONAL SHAPES NOW (each = one field/hook; skipping = a post-freeze contract amendment + a `.bnn` migration).** Owner rulings ③④⑤. **(a)** `SubShapeRef` `kind: 'vertex'` reserved in the role grammar (a corner an annotation or a Miqdar node binds to) — the one contract that must never move. **(b)** `Element.phase?` reserved (existing/demolition/new — Planitor sequences by it; unset ⇒ `new`). **(c)** `ParamField.relevantWhen?` reserved (conditional fields; absent ⇒ always shown, so existing schemas are unaffected). Landed together in P5 step 0g. |
...
## Entry 27 — 2026-07-14 — Amer (local PC, Windows, real browser) — **P4 STEPS 2c/2d/4/10/11/12 + THE D19 EQUIVALENCE TEST AND ITS UI-REFRESH GAP — ALL DONE. THE VIEWPORT HAS EDGES, A CLICK PICKS A FACE, AND AN AGENT EDIT NOW REFRESHES THE VIEW. `pnpm verify` GREEN, 209 TESTS.**

**Task (owner):** continue P4 from Entry 26 — steps **2c** (retain provenance/edges), **2d** (coalesce the render path), **4** (sub-shape picking), **10/11/12** (the small ones), and **the D19 equivalence test** (P4 exit criterion) *plus* the UI-refresh gap Entry 26 §3 flagged that would make it fail as written. No kernel/protocol changes; one internal document-layer type narrowing (step 12).

### 1. WHAT CHANGED (apps/web unless noted)
- **2c — PROVENANCE/EDGES/BOUNDS ARE RETAINED, AND THE VIEWPORT HAS EDGES AT LAST.** `toBufferGeometry` dropped `provenance`, `edgePositions` and `bounds` on the floor (three comments promised them, zero code kept them — the exact "promised in prose, absent in code" signature §1 teaches). Now each `DrawnPart` keeps the whole `MeshBuffers` (its `positions`/`normals`/`indices` are already held alive by the geometry attributes, so it costs no extra memory); **edge polylines render as `THREE.LineSegments`** (each `EdgePolyline` expanded to segment pairs, indexed into the shared position buffer — one line object per part, batching deferred to step 9 like the faces); and the kernel's **tight `bounds`** become the geometry's `boundingBox`/`boundingSphere` directly. *(Rendered edges are a large part of why the viewport read as a toy — now it reads as CAD linework.)*
- **2d — THE RENDER PATH IS COALESCED.** `RenderGateway.tessellate` now takes a per-call `coalesceKey`; `Viewport.setScene` passes `render:tessellate:<nodeId>` per part, so a newer frame's tessellation **supersedes** the in-flight one in the kernel client instead of the `#frame` guard discarding results the kernel already finished computing. Namespaced off the document's `rebuild:<id>` key so they never collide on the shared client. A superseded/cancelled part resolves to `null` and the stale frame is dropped by the guard — geometry building is now deferred to AFTER the guard, so a superseded frame builds no GPU resources at all.
- **4 — SUB-SHAPE PICKING. A CLICK ON A FACE YIELDS A `SubShapeRef`, AND NO HUMAN TYPES A TOKEN.** `Viewport.pick(ndcX, ndcY)` raycasts the face meshes (never the edge lines), and the pure `resolveFacePick` (in `render/pick.ts`, no three.js ⇒ headless-testable) maps the hit triangle → owning face `SubShapeRef` through the 2c provenance. `ViewportCanvas` turns a click (pointer down+up within 4px; a drag stays OrbitControls') into a pick; `App` selects the element and shows the token. **Verified live: a canvas click resolved to `…finish/face/y-max#0`** — the exterior finish face a window hosts on. *This is the first end-to-end proof of P2's naming in a user's hands rather than a test.*
- **10 — THE TWO FAILURE STATES ARE VISIBLE.** New `ProblemsPanel` surfaces `doc.unbuildable()` (greyed, with reason) and `doc.brokenRefs()` (marked, with the unresolved token); the element picker tags an unbuildable element `⚠ unbuildable`. **Neither is ever offered a "fix" or "drop"** (D43 / domain rule 3). The old `if (parts === undefined) continue` stays (there is genuinely no geometry to draw) but the absence is now explained, not silent.
- **11 — STOPPED GREPPING ENGLISH PROSE FOR A SUPERSEDED FRAME (as far as apps/web can alone).** `isSuperseded` now keys on the **frozen typed-code marker** `[SUPERSEDED]` (which `KernelFailureError` renders from `failure.code`, not from the human sentence after it) instead of `/superseded/i`. This survives any message rewording AND stops swallowing a genuine failure whose prose merely contains the word. ⚠⚠ **THE CLEAN FIX IS STILL OWED AND IT IS ZAYD'S + OWNER'S:** preserve the kernel `SUPERSEDED` code across the document boundary — capture the `KernelFailureCode` on a failed `ElementGeometry` in `build.ts`, carry it on `StagedRebuild.failure` in `document.ts`, add a `SUPERSEDED` `CommandFailure` code (or a `cause`) in `commands.ts`, then `error.code === 'SUPERSEDED'` here and delete the string match. **`CommandFailure`'s code set FREEZES AT P5 — free now, an amendment later.** The precise recipe is in a comment on `isSuperseded`. I did **not** touch that contract solo (Zayd's package; the prompt flagged it for coordination).
- **12 — `GeometryGateway` NOW EXCLUDES `tessellate` AT THE TYPE LEVEL** (`packages/document/src/geometry.ts`): `request<Op extends DocumentOpName>` where `DocumentOpName = Exclude<OpName, 'tessellate'>`. The comment claimed it; the type admitted it (`g.request('tessellate', …)` typechecked, proved by the review). Internal type narrowing only — **no contract/protocol change**; nothing in the document layer calls `tessellate`, and the renderer reaches it through its own `RenderGateway`. Machine-checked by a `@ts-expect-error` in `d19-boundary.test.ts` (revert the `Exclude` → `tsc` goes red with "unused '@ts-expect-error'", proven).
- **THE UI-REFRESH GAP (Entry 26 §3) — CLOSED.** `window.bunyan` is now wrapped by `withUiRefresh(agent, bump)`: the agent surface's mutators (`execute`/`undo`/`redo`) bump App's `version` so an agent edit refreshes the view exactly like a human edit — `dryRun`/reads do not (nothing changed), a rejected `execute` does not (reject + keep-last-good). `DocumentContext` has no change signal and adding one is a P5-frozen contract addition, so the fix is the app-level equivalent: funnel BOTH surfaces through one notify.

### 2. PROVEN
- **`pnpm verify` GREEN — all five CI steps** (typecheck incl. apps/web · lint · format:check · **209/209 tests** · reseed skips off-PR). Was 194; **+15**: pick (4), agentRefresh (4), `isSuperseded` (4), d19-equivalence (2), the step-12 type check (1).
- **THE D19 EQUIVALENCE TEST EXISTS** (`tests/d19-equivalence.test.ts`, real OCCT): the SAME scripted end-to-end edit (material→style→wall→setParams→undo) driven through `createAgentSurface` **and** through direct `doc.execute` (what `App.dispatch` does) lands on the **same UndoableEdits and the same final scene**, id- and timestamp-normalised (ULIDs are random per run — that is exactly what must NOT matter). Plus: every `CORE_COMMANDS` verb is reachable through the agent surface (no back-door verbs).
- **⚠⚠ LIVE IN THE REAL BROWSER (the method that catches what tests do not):** booted on the real OCCT kernel; (a) authored a 2nd wall through `window.bunyan.execute` alone and **the Element picker appeared with two walls with NO UI action** — the refresh gap is closed (before this, the picker would not have appeared); (b) a synthetic canvas click resolved to a valid `SubShapeRef` face token in the read-out; (c) zero console errors/warnings across the session. *(The in-app screenshot still times out on the WebGL canvas — known local limit; functional proofs stand. A draw-call hook to count line vs triangle primitives was inconclusive — three's context handle differs from a re-`getContext` view — so edges are verified by construction: the kernel populates `edgePositions`/`provenance.edges` (`occt-kernel.test.ts:115` asserts 12 for a box), `buildEdgeSegments` reads exactly those, and the working pick proves the same retained buffers are sound.)*

### 3. ⚠ WHAT IS OWED NEXT (still Amer, still P4/P4.5)
- **STEP 11's CLEAN FIX** — the document-layer typed `SUPERSEDED` propagation above. **Zayd's package + a P5-frozen contract ⇒ owner-gated, coordinate with Zayd.** Recipe is in the `isSuperseded` comment.
- **STEP 9 — THE SCALE HARNESS** (the four numbers; heap/solid is the dangerous unknown). Still a measure-then-design gate, not started as coding (correctly, per the prompt).
- **The remaining P4 steps: 1** (WebGPU + WebGL2 fallback), **7** (TSL shading), **8** (keyboard undo/redo — there is still not one `keydown` handler), **storage** (step 5 / `StorageAdapter` over FSA/OPFS/IndexedDB), and **P4.5** (the interaction model — the Architect's baseline-Wall parameterisation gates it; do not start snapping/the wall tool).
...
## Entry 32 — 2026-07-16 — Zayd (dev box, headless) — **THE OWNER RULED THE FIVE FREEZE-GATE DECISIONS (D52–D54). THE FREEZE GATE IS NOW UNBLOCKED; NEXT ENTRY IMPLEMENTS D50 STEP 0.**

The Freeze Gate's own close-order made ①–⑤ the first thing to settle — *"minutes of decision, days of foreclosed cost"*, and several are permanent contract **shapes** that a guess could not undo. The owner ruled all five in one pass:

| # | Question | ✅ RULING | Decision |
|---|---|---|---|
| **①** | Wall parameterisation | **Baseline `{start, end}`** — length & height DERIVED (`start`/`end`/`baseLevel`/`topLevel`). | **D52** |
| **②** | Where constraints live | **A first-class `scene.json` `constraints` collection**, orthogonal to `params` — `ParamSchema` gains no driven-dimension concept. | **D53** |
| **③** | `SubShapeRef kind:'vertex'` | **Reserve it** (role grammar carries a vertex form; kernel exports it when a consumer asks). | **D54a** |
| **④** | `Element.phase` | **Reserve it** (existing/demolition/new; unset ⇒ `new`). | **D54b** |
| **⑤** | `ParamField.relevantWhen` | **Reserve it** (conditional fields; absent ⇒ always shown). | **D54c** |

**Why each ruling is the load-bearing one, not the convenient one:**

- **D52 is what makes the rest of D50 *expressible*, not just tidier.** The three things D50 must deliver — *drag the end*, *wall joins* (§0c, anti-fuse), *grid-hosting* (§0b) — are all **impossible on a `{length}`+placement wall**: "drag the end" is a compound length+placement edit no command expresses, and a join/host has nothing to attach to. On a baseline they are, respectively, `setParams{end}`, "two baselines meet", and "snap `start`/`end` to an axis". The alternative did not just cost ergonomics; it foreclosed the phase's exit criteria.
- **D53 keeps `ParamSchema` a value schema.** Putting constraints *in* params would have forced a "driven/driving dimension" concept **into the contract that freezes at P5** and made the dependency graph something every consumer reconstructs by scanning params. A first-class `constraints` collection is the same typed structure the rebuild invalidator needs anyway (P5 s0a) — one graph, read by the invalidator and by Miqdar/Planitor alike.
- **D54 a/b/c are pure insurance** — each is one optional field/hook, and each alternative is *"amend a frozen contract + migrate every issued `.bnn`"*. Reserving costs nothing to existing schemas (all three default to today's behaviour) and buys the whole v1.0.x annotation / phasing / conditional-UI surface without a breaking change.

**Still open below the rulings** (unchanged by this entry):

- **⑥ THE CLEAN DELTA JSON SCHEMA — BLOCKED, escalation.** The BIMsync spec is not on this box (D37). It does **not** block 0a–0f; it blocks only the `UndoableEdit`/`SceneChange`/journal freeze. **Owner/Architect action: get the spec onto the box, or sign off freezing the transport shapes on a written guess.**

**NEXT ENTRY — implementation begins.** Per the close-order, with ①–⑤ ruled: **build D50 step 0**, staged — the **typed dependency graph (0a)** first (replacing `#touched()`, which is the seam and carries Entry 24b's latent `container→element` invalidation bug), then **hosting/associativity (0b, incl. `BuildContext.grid()`/base-top datums ⓐ)** on the new `{start,end}` wall, then **CRUD + generalised-D51 refuse-or-retarget (0e/0f, incl. ⓓ)**, then the **reserve-the-shapes pass (0g)** landing D54 a/b/c + ⓒ/ⓕ/ⓖ, with the **sketch solver (0d)** last (evaluate `planegcs` before writing one). Each clause of the constraint model is a separate exit-criteria test (move a level → the building follows; move a grid line → its columns follow; a baseline wall spans L1→L2 with derived height and its openings survive; two walls join and every hosted window still resolves; a sketch solves and `lateral.k` still names the drawn segment).
...
## Entry 37 — 2026-07-17 — Zayd (dev box, headless) — **D50 STEP 0g BUILT: THE "RESERVE THE SHAPES" PASS. EVERY PRE-FREEZE RESERVATION IS LANDED — PLUS AN OWNER-ORDERED REVIT-PARITY/AGENT SWEEP, AND SPACE EXTENT RULED (OPTION B → A SECOND SOLVER IN v1.0.0).**

**Task (owner):** resume implementation in the right direction. Per the Entry-36 handoff, the next step was **0g** — the last additive reservations before `SubShapeRef`/`Element`/`ParamField`/`BimObjectType`/`Scene` FREEZE at step 6. **Designed first** (`P5_step0g_design.md`, rev. 3, owner-ruled in two question rounds), then built. The freeze is the one irreversible act (a wrong shape after it costs an amendment across three products + every `.bnn` in the field), so this got a design + owner sign-off, not just a commit.

**Two new owner decisions (definitions, for the index):**
- **D55 — Space extent = Option B (room-bounding).** A `Space`'s boundary is DERIVED from its bounding walls (+ room separators), never stored; only its identity inputs freeze (`location` seed + Revit vertical model). **The room-bounding solver ships in v1.0.0** (not v1.0.x). Room separation lines are reserved now as a first-class **polyline** collection.
- **D56 — the pre-freeze reservation set.** Phasing is TWO datums (`phaseCreated?`/`phaseDemolished?`, refining D54b); and the reservations ⓒ `ifcMapping` · ⓕ `migrateStyle` · ⓚ `georeference` · ⓜ `formula` · ⓝ `parentElementId` **plus the owner-ordered Revit-parity/agent sweep** ⓟ `properties` (Pset/agent bag) · ⓠ `classifications` · ⓡ `mark` · ⓢ `Grid.geometry` all land as optional/additive fields.

**The owner ruled, across two rounds (D55/D56):**

- **Space extent → OPTION B** (derived from bounding walls — Revit's room-bounding solve), **the "beat-Revit" choice.** ⚠⚠ **And the owner ruled the solver ships in v1.0.0, not v1.0.x** — so **v1.0.0 now carries TWO heavy, formerly-unbudgeted solvers** (the sketch-constraint solver 0d **and** the room-bounding solver). Both were north-star hooks; D50 + this ruling pulled them into scope. **The schedule must be re-cut; step 1 now owns the room-bounding solver.**
- **Phasing → TWO datums** (`phaseCreated?`/`phaseDemolished?` — Revit's model; Planitor demolition sequencing needs the second; one field was a guaranteed post-freeze `Element` amendment).
- **Round-2 batch → land all five, AND run a fresh Revit-parity + agent-friendly sweep** — which found four more (ⓟ/ⓠ/ⓡ/ⓢ), all reserved.
- **Room separators → RESERVE NOW** as a first-class **polyline** collection (owner chose a chain over a single segment).

**What was built (`@bunyan/document` + `@bunyan/protocol`, all optional/additive — no behaviour change, no `SCENE_SCHEMA_VERSION` bump):**

| Row | Shape landed | File |
|---|---|---|
| ③ vertex | reserved role grammar documented (the `kind` already round-trips) | `protocol/subshape.ts` |
| ④+ⓛ phasing | `Element.phaseCreated?` / `phaseDemolished?` | `entities.ts` |
| ⓝ nesting | `Element.parentElementId?` | `entities.ts` |
| ⓟ properties | `Element.properties?` (Pset/agent bag — the unschematised IFC/agent lane) | `entities.ts` |
| ⓠ classifications | `Element.classifications?` (Uniclass/OmniClass/AssemblyCode) | `entities.ts` |
| ⓡ mark | `Element.mark?` (schedule/tag identifier) | `entities.ts` |
| ⑤ / ⓜ | `ParamField.relevantWhen?` + `ParamCondition`, `ParamField.formula?` | `schema.ts` |
| ⓒ / ⓕ | `BimObjectType.ifcMapping?` + `IfcMapping`, `BimObjectType.migrateStyle?` | `types.ts` |
| ⓚ georeference | `Scene.georeference?` + `ProjectGeoreference` | `scene.ts` |
| ⓢ grid geometry | `Grid.geometry?` + `GridGeometry` (`line`\|`arc` tagged union) | `entities.ts` |
| Space-B (D55) | `SpatialContainer.location?`/`baseOffset?`/`upperLevelId?`/`limitOffset?` — **boundary DERIVED, never stored** (recipe-is-truth) | `entities.ts` |
| Separators (D55) | `RoomSeparator` (polyline) + `Scene.roomSeparators` collection + its `SceneCollection` member + dependency edge | `entities.ts` / `scene.ts` / `dependency.ts` |

- ⚠ **`roomSeparators` tripped 0a's exhaustive switch** (as designed) — the file did not compile until its dependency edge was declared. It is a **declared "nothing"** for element geometry: a separator re-bounds a Space (a query the solver recomputes), never an element's solid — the same shape the `materials` edge has.
- ⚠ **Hardened the hostile-`.bnn` guard for `constraints` (a LATENT 0b gap) and `roomSeparators`** — a sent file with `"constraints": null` or `"roomSeparators": null` would have overwritten `emptyScene()`'s `{}` default and crashed deeper in (`typeof null === 'object'`). Now a typed refusal, like the other collections.

**Test (`tests/reserve-shapes.test.ts`, 15, PURE — no kernel, because 0g touches no geometry):** freeze-safety type checks (every field optional + absent-able; the tagged unions extend by ADDING members — a `@ts-expect-error` pins that a Space stores no `boundary`); a **real-codec round-trip** (`saveBnn`→`loadBnn`) of a scene carrying every reserved field is byte-identical, and one carrying none defaults `roomSeparators` to `{}` / `georeference` absent; the `roomSeparators` dependency edge returns `[]`; and the two hostile-`null` guards refuse.

**Verification:** full suite **262 green** (35 files, was 247); `pnpm typecheck`/`lint`/`format:check`/`test`/`reseed:check` all clean on this box.

**Freeze-Gate status:** ①–⑤ ruled · **0a ✅ · 0b ✅ · 0e/0f ✅ · 0g ✅**. Remaining in step 0: **0d** sketch solver (last; evaluate `planegcs`) and **0c** joins (with the real Wall — step 3 — anti-fuse absolute). **Next in the close-order: the TYPES (steps 1–5), starting with the spatial container tree (step 1), which now OWNS the room-bounding solver.** Step 5 (Opening) still must resolve ⓙ (a hosted type that both cuts a void and builds parts). ⑥ Clean Delta still blocked (BIMsync spec off-box). **All step-0 commits — including this one — are staged and owner-gated.**

### FILES
CHANGED: `packages/document/src/entities.ts` (Space-B extent, `RoomSeparator`, `GridGeometry`+`Grid.geometry`, `Element` phasing/nesting/properties/classifications/mark, `RoomSeparatorId`) · `scene.ts` (`roomSeparators` collection + `georeference`/`ProjectGeoreference` + `SceneCollection` member + version note) · `dependency.ts` (`roomSeparators` edge) · `schema.ts` (`relevantWhen`/`formula`/`ParamCondition`) · `types.ts` (`ifcMapping`/`IfcMapping`/`migrateStyle`) · `bnn.ts` (guard hardened for `constraints`+`roomSeparators`) · `packages/protocol/src/subshape.ts` (vertex reservation note) · `tests/reserve-shapes.test.ts` (**new**, 15) · `v1.0.0_imp_plan.md` (0g ✅ BUILT; close-order + NEXT AGENT + schedule re-cut) · `P5_step0g_design.md` (**new**, the design) · `current_state.md` (this entry + D55/D56 in §4). **Green; commit is owner-gated.**

---

## Entry 38 — 2026-07-18 — Zayd (dev box, headless) — **0g.2 BUILT: THE VERB HALF (ⓣ). THE RESERVED METADATA NOW HAS AN AUTHORING PATH — BEFORE `Command.argsSchema` FREEZES. PLUS ⓤ RESERVED, ⓥ RECORDED. 0g IS FULLY CLOSED.**

**Task (owner):** resume implementing the v1.0.0 plan. Per the close-order, 0g was **data-shapes-done but reopened** — the plan's own 0g pre-freeze review (2026-07-18, Freeze-Gate row **ⓣ**) had found the load-bearing miss and folded the fix into the plan. This entry executes it. **No new owner decision was needed** — ⓣ mechanically applies the already-ruled ⓓ discipline (*"the retarget/ack arg must be in the `argsSchema` before it freezes"*) to the CREATE path, for the fields D56 already reserved.

> ## ⚠⚠ THE GAP: 0g RESERVED THE NOUNS, NOT THE VERB ARGS THAT SET THEM — AND `Command.argsSchema` FREEZES AT THE SAME STEP 6.
>
> Entry 37 reserved six optional `Element` fields (`mark`, `properties`, `classifications`, `phaseCreated`, `phaseDemolished`, `parentElementId`). But **no command could author them**: `createElement.argsSchema` was a fixed key list, `setClassification` sets only `{ifcClass, loadBearing}`, and there was no setter for the rest. ⇒ when a body lands later (P6 writes `properties`; 2D writes `mark`; Planitor writes `phase*`), the author would **amend a frozen `argsSchema`** — a frozen-contract + agent-tool-list change — **or** accept create-then-set forever (an element can never be *born* with a mark/phase in one atomic edit). *A reserved slot with no authoring path is not reserved; it is decorative — the ⓡ/ⓟ half of 0g, unfinished.*

**What was built (`@bunyan/document`, additive — no protocol change, no `SCENE_SCHEMA_VERSION` bump):**

- **ⓣ — `createElement` authors the six.** The reserved fields landed as **optional args on `createElement.argsSchema`**, threaded in `execute` exactly like `name`/`loadBearing`/`containerId`. An element is now **born with its mark/phase/properties/classifications in ONE atomic edit.** ⚠ `parentElementId` is a **validated ref** — a dangling parent refuses (`NOT_FOUND`), the `containerId` lesson generalised (a reserved field is still a ref that must resolve).
- **ⓣ — new verb `core.setElementMetadata`** (registered in `CORE_COMMANDS`) for post-create editing. **Semantics:** a PROVIDED arg sets the field; an ABSENT one leaves it unchanged; **clearing is a future *additive* arg, never a `null` overload** (a value-vs-cleared ambiguity is exactly what a frozen contract must not carry). **No refuse-or-retarget guard and no rebuild** — none of these six is identity- or quantity-bearing (unlike a style-layer name or a `materialId` — D51/ⓓ), and none feeds the build, so it re-stages nothing (`rebuilt: []`, like `setClassification`).
- **ⓤ — RESERVED (not merely recorded):** `properties?`/`classifications?` landed on **`SpatialContainer` AND `Grid`**, mirroring `Element` (ⓟ/ⓠ). P6 imports `IfcSpace`/`IfcBuildingStorey`/`IfcSite`/`IfcGrid` psets (Pset_SpaceCommon: area, occupancy, fire compartment) that would otherwise be silently dropped. Purely additive, provably migration-free. ⚠ **This is the one owner-flaggable call in the batch** — the plan marked it "reserve now OR record as deferrable." I reserved it (under-reserving before an irreversible freeze is strictly riskier than reserving, and it mirrors the owner-approved Element treatment), **but it is trivially deferrable** — a one-line revert per interface if the owner prefers to add it post-freeze.
- **ⓥ — RECORDED:** the invariant *"when `Grid.geometry` is present, `axis`+`offset` are a non-positional nominal label — never read for placement"* was already in the `Grid.geometry` doc (Entry 37); strengthened with an explicit **three-consumer note** (Miqdar/Planitor/2D tagger) so a field-support bug (reading `axis` on a radial grid) is contract-flagged, not latent.

**Test (`tests/reserve-shapes.test.ts`, +6 → 21, PURE — no kernel, driven through a minimal hand-built `CommandContext`):** an element **born** with mark+phase+properties+classifications in ONE `createElement` edit, then `saveBnn`→`loadBnn` **byte-identical**; a no-metadata create carries NONE (absent, not empty); a **dangling `parentElementId` refuses**; `core.setElementMetadata` sets-provided/leaves-absent and re-stages nothing; the ⓤ container/grid IFC bags round-trip; and a **structural revert-check** (the six args are in the schema). ⚠ **REVERT-VERIFIED** (the standing rule): dropping `mark` from `createElement.argsSchema` makes the born-with test fail (`INVALID_ARGS` — the arg refused as an unknown key); restored ⇒ green. *A fix without a test that fails in its absence is an assertion.*

**Verification:** full suite **268 green** (35 files, was 262); `pnpm typecheck`/`lint`/`format:check`/`test`/`reseed:check` all clean on this box. ⚠ **`format:check` caught two files on the first run** (prettier, before the tests ever ran) — the Entry-14/24 lesson, working as designed; fixed with `prettier --write`.

**Contract note (§2):** `Command` is still release-candidate (freezes at P5). `createElement.argsSchema` gained six **optional** args; `core.setElementMetadata` is a new command **instance** (additive — D19; adding a command is not a pre-freeze contract change, but the *shape* of the args it leans on freezes at step 6, which is why it is reserved now). `SpatialContainer`/`Grid` gained two optional fields each. No frozen kernel-protocol change; no `.bnn` migration (every existing file loads unchanged).

**Freeze-Gate status:** ①–⑤ ruled · **0a ✅ · 0b ✅ · 0e/0f ✅ · 0g ✅ (data shapes + verb half, fully closed)**. Remaining in step 0: **0d** sketch solver (last; evaluate `planegcs`) and **0c** joins (with the real Wall — step 3 — anti-fuse absolute). 🔴 **HARD GATE before step 1: price the two solvers** (sketch-constraint 0d + room-bounding, both v1.0.0) and state the number to the Architect — the direction is ruled (D55), the estimate is owed. **Next in the close-order after that gate: the TYPES (steps 1–5)**, step 1 owning the room-bounding solver; **step 5 (Opening) must resolve ⓙ** (a hosted type that both cuts a void AND builds parts). ⑥ Clean Delta still blocked (BIMsync spec off-box). **This commit — like all step-0 work since Entry 33 — is staged and owner-gated.**

### FILES
CHANGED: `packages/document/src/commands.ts` (six optional args on `createElement.argsSchema` + threading + `parentElementId` ref-validation; new `setElementMetadataCommand`; registered in `CORE_COMMANDS`) · `packages/document/src/entities.ts` (ⓤ `properties?`/`classifications?` on `SpatialContainer` + `Grid`; ⓥ three-consumer invariant note on `Grid.geometry`) · `tests/reserve-shapes.test.ts` (+6: the verb half, the ⓤ round-trip, the revert-check) · `v1.0.0_imp_plan.md` (0g.2 ✅ BUILT; Freeze-Gate ⓣ/ⓤ/ⓥ closed; close-order + NEXT AGENT updated) · `current_state.md` (this entry). **Green; commit is owner-gated.**

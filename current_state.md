...
**NEXT:**
- **Owner:** **the FREEZE (step 6) is still the owner's act and still unblocked.** D74 moved no frozen byte, bumped no `SCENE_SCHEMA_VERSION`, added no field or verb; the `exposedRefs` work is Type-local population of a member D72 already reserved. Entry 62 is owner-gated for commit/push as usual. ⚠ **One question parked for you, not blocking: should `undo`/`redo` become Commands (rule 9)?** Additive either way.

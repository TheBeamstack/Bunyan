...
8. **⚠⚠ A NEW RULE BINDS THE NEXT CONSUMER AND NOTHING ELSE — SWEEP IT *BACKWARD* (Entry 59, D68).** The
   ...
   **Only rule 6 came back clean, and it is the only one that was tested by DELETING it and rebuilding
   rather than by reading.** ⇒ **The sweep costs ~30 minutes per rule and its hit rate is 3-in-4. Rules
   1–4, 7–11, 13–15, 17 and 18 have never been swept at all.** The two with the most to lose are **13**
   (an id survives every rebuild, resize and re-issue — now with D59 derived children whose ids encode a
   slot) and **15** (measured, never reconstructed; omitted, never zeroed).
...
`materialId` at contract 1.2 D70; the codec seam dispatches and `.bnn` is registered through it D71). ⚠ Rules 1–4,
7–11, 13–15, 17, 18 remain UNSWEPT — that is a live to-do, not a clean bill** · D50 step
...
- **⚠ OWED CROSS-PROJECT (D70): `../Planitor/v2.2_spec.md` §4 must be updated to `contract_version 1.2` with `parts[].materialId` required.** Bunyan's producer and its published schema now emit it; Planitor's spec is the other half of that contract and is currently out of step.
